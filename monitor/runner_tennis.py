...
from .alerts import build_tennis_global_alerts
...
def is_player_match(player: str, text: str) -> bool:
    player_clean = clean_for_matching(player)
    text_clean = clean_for_matching(text)
    if not player_clean or not text_clean:
        return False
    if fuzz.token_set_ratio(player_clean, text_clean) >= 78:
        return True

    player_tokens = player_clean.split()
    text_tokens = set(text_clean.split())
    if not player_tokens:
        return False
    if len(player_tokens) == 1 and player_tokens[0] in text_tokens:
        return True
    if all(token in text_tokens for token in player_tokens):
        return True

    last_name = player_tokens[-1]
    return len(last_name) > 2 and last_name in text_tokens

def has_vs_marker(text: str) -> bool:
    text = str(text).lower().replace("vs.", "vs")
    return " vs " in text or " v " in text

def is_matchup_match(home: str, away: str, poly_text: str) -> bool:
    return has_vs_marker(poly_text) and is_player_match(home, poly_text) and is_player_match(away, poly_text)

def resolve_outcome_player(outcome: str, home: str, away: str) -> Optional[str]:
    home_score, away_score = match_score(outcome, home), match_score(outcome, away)
    if home_score >= 78 and home_score > away_score:
        return home
    if away_score >= 78 and away_score > home_score:
        return away
    if is_player_match(outcome, home) and not is_player_match(outcome, away):
        return home
    if is_player_match(outcome, away) and not is_player_match(outcome, home):
        return away
    return None

def get_h2h_odds(bookie: dict, selection: str) -> Optional[Decimal]:
    for name, odds in bookie.get("h2h", {}).items():
        if is_player_match(selection, name) or is_player_match(name, selection):
            return odds
    return None
...
def run_tennis() -> None:
    ...
    try:
        logger.info("📡 Initializing Tennis Sniper...")
        raw_odds, raw_poly = clients.get_tennis_fiat_data(), clients.get_tennis_polymarket_events()
        logger.info(f"   [INFO] Tennis Odds API returned {len(raw_odds)} events.")
        logger.info(f"   [INFO] Polymarket returned {len(raw_poly)} tennis-tagged events.")
        ...
        for game in raw_odds:
            ...
            if k not in fiat_games:
                fiat_games[k] = {
                    ...
                    "sport_key": game.get('sport_key', 'tennis'), "bookies": []
                }
            ...
            for b in game.get("bookmakers", []):
                ...
                if b_data["h2h"] or b_data["totals"]:
        ...
        for gk, x in fiat_games.items():
            ...
            # 1. Fiat Scanner (Tennis - Match Winner & Match Game Totals)
            for i in range(len(x["bookies"])):
                for j in range(i + 1, len(x["bookies"])):
                    b1, b2 = x["bookies"][i], x["bookies"][j]
                    for sel, o1 in b1["h2h"].items():
                        matched = resolve_outcome_player(sel, h, a)
                        if not matched: continue
                        opp_player = a if matched == h else h
                        o2 = get_h2h_odds(b2, opp_player)
                        if o1 and o2:
                            imp = (Decimal("1")/o1) + (Decimal("1")/o2)
                            if imp < 1:
                                roi = round(((1/float(imp))-1)*100, 2)
                                if 0 < roi < TENNIS_MAX_ROI:
                                    fiat_opportunities.append(_build_fiat_opp(x, b1["name"], b2["name"], o1, o2, "Match Winner", matched, opp_player, imp, roi))
                    ...
                    for pt, t1_odds in b1.get("totals", {}).items():
                        ...
                        if o1_over and o2_under:
                            ...
                            if imp < 1:
                                roi = round(((1/float(imp))-1)*100, 2)
                                if 0 < roi < TENNIS_MAX_ROI: fiat_opportunities.append(_build_fiat_opp(x, b1["name"], b2["name"], o1_over, o2_under, f"Match Games Total {pt}", "Over", "Under", imp, roi))
                        if o1_under and o2_over:
                            imp = (Decimal("1")/o1_under) + (Decimal("1")/o2_over)
                            if imp < 1:
                                roi = round(((1/float(imp))-1)*100, 2)
                                if 0 < roi < TENNIS_MAX_ROI: fiat_opportunities.append(_build_fiat_opp(x, b1["name"], b2["name"], o1_under, o2_over, f"Match Games Total {pt}", "Under", "Over", imp, roi))

            # 2. Poly Scanner (Tennis - Match Winner & Full Match Game Totals)
            target_markets = []
            for e in raw_poly:
            ...
            for b in x["bookies"]:
                for target, m in target_markets:
                    ...
                    if not outs or not toks: continue

                    if mt == 'moneyline':
                        for idx, outcome in enumerate(outs):
                            if idx >= len(toks): continue
                            matched = resolve_outcome_player(str(outcome), h, a)
                            if not matched: continue
                            opp_player = a if matched == h else h
                            f_opp = get_h2h_odds(b, opp_player)
                            if f_opp:
                                ...
                                logger.info(f"   [ML] {b['name']:<11} | {str(outcome)[:10]:<10} | {b['name']} Opp: {float(f_opp):<5} | Poly Ask: {poly_price:<5} | Status: {'✅' if hedge.passes_liquidity_filter else '❌ ' + str(hedge.reject_reason)}")
                                if hedge.passes_liquidity_filter:
                                    roi = round(float((hedge.locked_profit/hedge.total_outlay)*100), 2)
                                    if 0 < roi < TENNIS_MAX_ROI:
                                        opportunities.append(_build_opp(x, b["name"], f_opp, hedge, "Match Winner", str(outcome), opp_player, roi, 0.0, 0.0))

                    elif mt == 'tennis_match_totals':
                        line_match = re.search(r'(?:o/u|over/under)\s*(\d+(?:\.\d+)?)', market_context)
                        if not line_match: continue
                        line = round(float(line_match.group(1)), 1)
                        ...
                        for idx, out_lbl in enumerate(outs):
                            ...
                            if f_opp:
                                ...
                                hedge = evaluate_buy_hedge_from_asks(book.get("asks", []), f_opp)
                                poly_price = f"${float(hedge.best_ask):.2f}" if hedge.best_ask else "N/A"
                                logger.info(f"   [TOT] {b['name']:<10} | {poly_side[:10]:<10} | {b['name']} Opp: {float(f_opp):<5} | Poly Ask: {poly_price:<5} | Status: {'✅' if hedge.passes_liquidity_filter else '❌ ' + str(hedge.reject_reason)}")
                                if hedge.passes_liquidity_filter:
                                    roi = round(float((hedge.locked_profit/hedge.total_outlay)*100), 2)
                                    if 0 < roi < TENNIS_MAX_ROI:
                                        opportunities.append(_build_opp(x, b["name"], f_opp, hedge, f"Match Games Total {line}", poly_side, fiat_side, roi, 0.0, 0.0))

        logger.info("\n" + "="*80)
        final_alerts = build_tennis_global_alerts(opportunities, fiat_opportunities, limit=3)
        for msg in final_alerts: clients.send_telegram_alert(msg)
        logger.info(f"✅ TENNIS SCAN COMPLETE. Sent {len(final_alerts)} alerts.")
        logger.info("="*80)
    finally:
        clients.close()
...
def _build_fiat_opp(x, b1, b2, o1, o2, m, s1, s2, imp, roi):
    ...
    return FiatArbitrageOpportunity("tennis", x['home'], x['away'], format_to_local(x['time']), m, b1, s1, float(o1), (payout/float(o1)), b2, s2, float(o2), (payout/float(o2)), float(imp), payout, roi)

def _build_opp(x, b, f_o, hedge, m, p_s, f_s, roi, dt, sp):
    return ArbitrageOpportunity("tennis", x['home'], x['away'], format_to_local(x['time']), m, p_s, f_s, b, float(f_o), float(hedge.shares), float(hedge.vwap or 0), float(hedge.marginal_price or 0), float(hedge.poly_spend), float(hedge.poly_fees), float(hedge.sportsbook_stake), float(hedge.total_outlay), float(hedge.locked_profit), roi, dt, sp)
